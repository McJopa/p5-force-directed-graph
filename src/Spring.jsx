...
const Spring = () => {
  ...
  const sketch = (p) => {
    ...
    p.setup = () => {
      ...
      const root = new Mover.Mover(0, 0, 50, p.color(0, 255, 255), 500, p);
      ...
      secondaryList.forEach((secondary) => {
        for (let i = 0; i < 5; i++) {
          ...
          p.balls.push(child);
          const connection = new Connection(secondary, child, 20, 40, true, p);
          p.connections.push(connection);
        }
      });
    };
    ...
    p.draw = () => {
      ...
      for (let i = 0; i < rest.length; i++) {
        for (let j = 0; j < rest.length; j++) {
          if (i !== j) {
            const dir = p5.Vector.sub(rest[j].position, rest[i].position);
      ...
      p.balls.forEach((ball) => {
        ...
        p.fill(255);
        p.text(
          `x:${Math.floor(ball.position.x)} y:${Math.floor(ball.position.y)}`,
          ball.position.x,
          ball.position.y
        );
        let d = p.dist(
